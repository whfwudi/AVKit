...
//视频文件输出代理
@interface HYVideoRecordViewController ()<AVCaptureFileOutputRecordingDelegate>
//负责输入和输出设置之间的数据传递
...
//聚光光标
@property (nonatomic, weak) IBOutlet UIImageView *focusCursor;
//计时器
...
@implementation HYVideoRecordViewController
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if (error) {
        NSLog(@"获得设备输入对象出错,错误原因:%@",error.localizedDescription);
        return;
    }
...
-(void)captureOutput:(AVCaptureFileOutput *)captureOutput didFinishRecordingToOutputFileAtURL:(NSURL *)outputFileURL fromConnections:(NSArray *)connections error:(NSError *)error
{
    ...
    if (_needSavedPhotosAlbum) {
        ALAssetsLibrary *assetsLibrary = [[ALAssetsLibrary alloc]init];
        [assetsLibrary writeVideoAtPathToSavedPhotosAlbum:outputFileURL completionBlock:^(NSURL *assetURL, NSError *error) {
            if (error) {
                NSLog(@"保存视频到相簿过程中发生错误，错误信息：%@",error.localizedDescription);
                [MBProgressHUD showError:@"保存视频到相簿失败." toView:self.view];
            }
            if (lastBackgroundTaskIdentifier != UIBackgroundTaskInvalid) {
                [[UIApplication sharedApplication] endBackgroundTask:lastBackgroundTaskIdentifier];
            }
            NSLog(@"成功保存视频到相簿.");
            [MBProgressHUD showSuccess:@"成功保存视频到相簿." toView:self.view];
        }];
    } else {
...
{
    self.navigationController ? [self.navigationController popViewControllerAnimated:YES] : [self dismissViewControllerAnimated:YES completion:NULL];
}
